...
class ForgeTest extends CIDatabaseTestCase {
	...
	public function testCompositeKey()
	{
		...
		elseif ($this->db->DBDriver === 'Sqlsrv')
		{
			$this->assertEquals($keys['pk_forge_test_1']->name, 'pk_forge_test_1');
			$this->assertEquals($keys['pk_forge_test_1']->fields, ['id']);
			$this->assertEquals($keys['pk_forge_test_1']->type, 'PRIMARY');

			$this->assertEquals($keys['forge_test_1_code_company']->name, 'forge_test_1_code_company');
			$this->assertEquals($keys['forge_test_1_code_company']->fields, ['code', 'company']);
			$this->assertEquals($keys['forge_test_1_code_company']->type, 'INDEX');

			$this->assertEquals($keys['forge_test_1_code_active']->name, 'forge_test_1_code_active');
			$this->assertEquals($keys['forge_test_1_code_active']->fields, ['code', 'active']);
			$this->assertEquals($keys['forge_test_1_code_active']->type, 'UNIQUE');
		}
