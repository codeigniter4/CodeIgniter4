...
final class IncomingRequestTest extends CIUnitTestCase
{
    ...
    public function testAutoGeneratedRequestId(): void
    {
        $this->assertInstanceOf(IncomingRequest::class, $this->request);
        $this->assertNotEmpty($this->request->getRequestId());
        $this->assertSame(32, strlen($this->request->getRequestId()));
    }

    public function testPassedRequestIdDoesntGetReplaced(): void
    {
        service('superglobals')->setServer('HTTP_X_REQUEST_ID', 'custom-value');

        $config  = new App();
        $request = new IncomingRequest($config, new SiteURI($config), null, new UserAgent());

        $this->assertSame('custom-value', $request->getRequestId());
    }
