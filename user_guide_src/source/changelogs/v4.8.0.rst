...
- ``IncomingRequest`` now generates a unique request ID for each request and provides it via the new ``getRequestId()`` method. If the request includes an ``X-Request-ID`` header, that value will be used as the request ID instead of generating a new one. See :ref:`request-id` for details.
