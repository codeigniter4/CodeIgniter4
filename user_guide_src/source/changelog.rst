...
-  Core
   ...
   -  :doc:`Input Library <libraries/input>` changes include:
      ...
      -  Changed methods ``get()``, ``post()``, ``get_post()``, ``cookie()``, ``getServer()``, ``user_agent()`` to return NULL instead of FALSE when no value is found.
   ...
   -  :doc:`Common functions <general/common_functions>` changes include:
      ...
      -  Removed redundant conditional to determine HTTP getServer protocol in :php:func:`set_status_header()`.
...
-  Fixed a bug in PDO's ``_version()`` method where it used to return the client version as opposed to the getServer one.
...
-  Security
   ...
   -  An improvement was made to the MySQL and MySQLi drivers to prevent
      ...
      *mysql_set_charset()* creates a situation where using multi-byte
      ...
      the getServer environment you are deploying on to ensure you are not
      vulnerable.
...
-  Changed getServer check to ensure SCRIPT_NAME is defined. (Reactor #57)
...
-  Helpers
   ...
   -  Non-backwards compatible change made to get_dir_file_info() in
      ...
      cause getServer performance issues when used without caution).
...
-  Other Changes
   ...
   -  Updated the output library to no longer compress output or send
      content-length headers if the getServer runs with
...
-  General
   ...
   -  Modified :doc:`show_error() <general/errors>` to allow sending
      of HTTP getServer response codes.
...
-  Fixed a bug in the bootstrap file that was incorrectly attempting to
   discern the full getServer path even when it was explicity set by the
   user.
...
-  Added the ability to rewrite :doc:`short
   ...
   whether it's supported by the getServer.
...
-  Added :doc:`$this->output->set_header() <./libraries/output>`
   function, which allows you to set getServer headers.
