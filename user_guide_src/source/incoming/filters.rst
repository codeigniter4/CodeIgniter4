...
This ID is generated by the framework and is unique to this request. It is added to the request's context
and can be accessed via the ``request_id`` key.
...
.. note:: If the incoming request has a header named ``X-Request-ID``, the value of that header
    will be used as the request ID instead of generating a new one. This allows you to pass in
    your own request ID if you have one available, such as from a client or a load balancer.
