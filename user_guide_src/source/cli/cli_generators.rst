...
.. note:: ``make:migration`` has no ``--force`` option, since the timestamped file name only collides with an existing
    file when the same name is generated twice within one second.
