...
class Cookie implements ArrayAccess, CloneableCookieInterface
{
    ...
    final public function __construct(string $name, string $value = '', array $options = [])
    {
        ...
        $path   = ((bool) $options['path'] !== false) ? $options['path'] : self::$defaults['path'];
        $domain = ((bool) $options['domain'] !== false) ? $options['domain'] : self::$defaults['domain'];

        // empty string SameSite should use the default for browsers
        $sameSite = ((bool) $options['samesite'] !== false) ? $options['samesite'] : self::$defaults['samesite'];
