...
class Cors implements FilterInterface
{
    ...
    public function before(RequestInterface $request, $arguments = null)
    {
        ...
        $response->setHeader('Access-Control-Allow-Headers', implode(', ', $corsConfig->headers));
        //  append  allow methods
        $response->setHeader('Access-Control-Allow-Methods', implode(', ', $corsConfig->methods));
        // append set credentials status and must be string
        $response->setHeader('Access-Control-Allow-Credentials', $corsConfig->credentials ? 'true' : 'false');
        // append set  max age
        $response->setheader('Access-Control-Max-Age', $corsConfig->maxAge);
        // append content type
        $response->setheader('Content-Type', implode('; ', $corsConfig->contentType));
