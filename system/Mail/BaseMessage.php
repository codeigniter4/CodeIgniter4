...
abstract class BaseMessage implements MessageInterface
{
	 ...
	 * @return $this
	 */
	public function attach($file, string $disposition = '', string $newname = null, string $mime = '')
	{
		if ($mime === '')
		{
			if (strpos($file, '://') === false && ! file_exists($file))
			{
				$this->handler->setErrorMessage(lang('email.attachmentMissing'), $file);
			...
			if (! $fp = @fopen($file, 'rb'))
			{
				$this->handler->setErrorMessage(lang('email.attachmentUnreadable', $file));

				return $this;
			}

			$fileContent = stream_get_contents($fp);
			$mime         = $this->determineMimeType(pathinfo($file, PATHINFO_EXTENSION));
			fclose($fp);
		}
		else
		{
	 ...
	 * @return mixed|string
	 */
	protected function determineMimeType(string $ext)
	{
		return Mimes::guessTypeFromExtension($ext) ?? 'application/x-unknown-content-type';
	}
